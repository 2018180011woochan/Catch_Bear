...
CTerrainPlayer::CTerrainPlayer(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, ID3D12RootSignature *pd3dGraphicsRootSignature, void *pContext)
{
	...
	CLoadedModelInfo* pAngrybotModel = CGameObject::LoadGeometryAndAnimationFromFile(pd3dDevice, pd3dCommandList, pd3dGraphicsRootSignature, "Model/EvilbearL2.bin", NULL);
	...
	m_pSkinnedAnimationController->m_ppAnimationSets[0]->m_ppAnimationSets[4]->m_nType = ANIMATION_TYPE_ONCE;
...
void CTerrainPlayer::Update(float fTimeElapsed)
{
	...
	else
	{
		...
		else SetTrackAnimationSet(0, 0);
	}
}

///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// 
CBearPlayer::CBearPlayer(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, void* pContext)
{
	m_pCamera = ChangeCamera(THIRD_PERSON_CAMERA, 0.0f);
	...
	CHeightMapTerrain* pTerrain = (CHeightMapTerrain*)pContext;
	SetPosition(XMFLOAT3(370.0f, pTerrain->GetHeight(370.0f, 650.0f) /*- 43.0f*/, 650.0f));

	if (pAngrybotModel) delete pAngrybotModel;
}

CBearPlayer::~CBearPlayer()
{
}

CCamera* CBearPlayer::ChangeCamera(DWORD nNewCameraMode, float fTimeElapsed)
{
	DWORD nCurrentCameraMode = (m_pCamera) ? m_pCamera->GetMode() : 0x00;
	if (nCurrentCameraMode == nNewCameraMode) return(m_pCamera);
	switch (nNewCameraMode)
...
void CBearPlayer::OnPlayerUpdateCallback(float fTimeElapsed)
{
	CHeightMapTerrain* pTerrain = (CHeightMapTerrain*)m_pPlayerUpdatedContext;
	XMFLOAT3 xmf3Scale = pTerrain->GetScale();
	XMFLOAT3 xmf3PlayerPosition = GetPosition();
	...
	if (xmf3PlayerPosition.y < fHeight)
	{
		...
		xmf3PlayerVelocity.y = 0.0f;
		SetVelocity(xmf3PlayerVelocity);
		xmf3PlayerPosition.y = fHeight/*-43.0f*/;
		SetPosition(xmf3PlayerPosition);
	}
}

void CBearPlayer::OnCameraUpdateCallback(float fTimeElapsed)
{
	CHeightMapTerrain* pTerrain = (CHeightMapTerrain*)m_pCameraUpdatedContext;
	XMFLOAT3 xmf3Scale = pTerrain->GetScale();
	XMFLOAT3 xmf3CameraPosition = m_pCamera->GetPosition();
...
void CBearPlayer::Update(float fTimeElapsed)
{
	CPlayer::Update(fTimeElapsed);

	float fLength = sqrtf(m_xmf3Velocity.x * m_xmf3Velocity.x + m_xmf3Velocity.z * m_xmf3Velocity.z);
	//SetTrackAnimationSet(0, ::IsZero(fLength) ? 0 : 1);
	if (m_eCurState == MOVE)	SetTrackAnimationSet(0, 1);
	else if (m_eCurState == IDLE)	SetTrackAnimationSet(0, 0);
	else if (m_eCurState == ATK)	SetTrackAnimationSet(0, 4);
}
